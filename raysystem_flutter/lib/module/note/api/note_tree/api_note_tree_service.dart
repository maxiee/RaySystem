...
class ApiNoteTreeService implements NoteTreeService {
  ...
  ApiNoteTreeService({
    ...
    IconData collapsedNoteIcon = Icons.description,
    IconData expandedNoteIcon = Icons.description,
  })  : _notesApi = notesApi,
        _debug = debug,
        _noteIcon = noteIcon,
  ...
  Future<List<NoteTreeItem>> getChildrenFor(int? parentId) async {
    ...
    if (_cache.containsKey(parentId)) {
      _logDebug(
          'Returning ${_cache[parentId]!.length} cached items for ${parentId ?? "root (null)"}');
    ...
    try {
      ...
      final items =
          _convertNoteTreeNodesToTreeItems(noteNodes, parentLevel + 1);

      // Cache the result
      _cache[parentId] = items;
      _logDebug(
          'Fetched ${items.length} items for parent ${parentId ?? "root (null)"}');
      return items;
    } catch (e) {
